...
nrf_driver_t nrf_driver = { 
  ...
  .user_config.retr_delay = ARD_1000US,
...
fn_status_t nrf_get_address(register_map_t reg, uint8_t *buffer, int size) {

  spi_manager_t *spi = &(nrf_driver.user_spi);
  spi_manager_init_spi(spi->instance, spi->baudrate);

  fn_status_t status;
  status = r_register_bytes(reg, buffer, size);
...
fn_status_t nrf_driver_send_packet(const void *tx_packet, size_t size) {
  ...
  while ((status_reg & 0x30) == 0) {
    status_reg = r_register_byte(STATUS);
  }
 
  w_register(STATUS, &reset, ONE_BYTE); 
  //if (status & 0x10)
  flush_tx_fifo();
  status = (fn_status_t) status_reg;
...
fn_status_t nrf_driver_create_client(nrf_client_t *client) {
  ...
  client->get_address = nrf_get_address;
...
//  for (size_t delay = ARD_250US; delay <= ARD_1000US; delay += 16)
  for (size_t delay = ARD_250US; delay <= 256; delay += 16)
  {
