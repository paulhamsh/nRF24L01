...
class NRF24L01:
    def __init__(self, spi, cs, ce, channel=80, payload_size=32):
        ...
        self.reg_write(DYNPD, 0b00111111)
        reg = self.reg_read(FEATURE)
        reg |= 0b0000100
        self.reg_write(FEATURE, reg)
        #self.reg_write(FEATURE, 0b00000100)
        # auto retransmit delay: 1750us
        # auto retransmit count: 8
        self.reg_write(SETUP_RETR, (6 << 4) | 8)
        #self.reg_write(SETUP_RETR, (6 << 4) | 15)
    ...
    def read_reg_bytes(self,reg):
        buf1 = bytearray(1)
        buf2 = bytearray(5)
     
        self.cs(0)
        self.spi.readinto(buf1, reg)
        self.spi.readinto(buf2)
        self.cs(1)
        return buf2
    
    def print_details(self):
        print("CONFIG      %2x" % self.reg_read(CONFIG))
        print("EN_RXADDR   %2x" % self.reg_read(EN_RXADDR))
        print("SETUP_AW    %2x"% self.reg_read(SETUP_AW))
        print("SETUP_RETR  %2x"% self.reg_read(SETUP_RETR))
        print("RF_CH       %2x"% self.reg_read(RF_CH))
        print("ERF_SETUP   %2x"% self.reg_read(RF_SETUP))
        print("STATUS      %2x"% self.reg_read(STATUS))
        print("RX_ADDR_P0  %2x"% self.reg_read(RX_ADDR_P0))
        print("TX_ADDR     %2x"% self.reg_read(TX_ADDR))
        print("RX_PW_P0    %2x"% self.reg_read(RX_PW_P0 ))
        print("FIFO_STATUS %2x"% self.reg_read(FIFO_STATUS))
        print("DYNPD       %2x"% self.reg_read(DYNPD))
        print("FEATURE     %2x"% self.reg_read(FEATURE))        

        print("RX_ADDR_P0  ", self.read_reg_bytes(RX_ADDR_P0))
        print("RX_ADDR_P1  ", self.read_reg_bytes(RX_ADDR_P1))
        print("TX_ADDR     ", self.read_reg_bytes(TX_ADDR))
